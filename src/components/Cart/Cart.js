...
function Cart() {
    ...
    const items = Object.entries(carts).map(([key, value]) => {
        ...
        return (
            <div key={key}>
                <img className="pic" src={value.image} alt="Img"></img> <span>{value.title}</span>: <span>{value.amount}</span>, price:<span>{value.price * value.amount}</span>
            </div>
        );
    })
    ...
    return (
        ...
        <div className="cart">
            ...
            <button className="reset" onClick={() => setCarts(!carts)} >Delete all products from the cart</button>
        </div >
    );
